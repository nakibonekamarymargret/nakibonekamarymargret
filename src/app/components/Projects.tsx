...
useEffect(()=>{
  const fetchProoject = async(()= {
      try{
        const response = await axios.get("http://localhost:7102/projects");
...
return (
    <div
      ...
      ></section>
        {project.featured && (
            <div
                className="bg-gradient-to-r from-yellow-400 to-orange-400 text-white text-xs font-bold px-4 py-1 flex items-center gap-1">
        ...
        {project.thumbnailUrl && (
            <div className="relative h-48 overflow-hidden">
              ...
              {project.status && (
                  <span
                      className="absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-medium bg-white/90 text-gray-800">
            {project.status}
          </span>
              )}
            </div>
        ...
        <div className="p-6 relative">
          ...
          <div
              className="absolute inset-0 bg-gradient-to-br from-blue-50 to-indigo-50 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
